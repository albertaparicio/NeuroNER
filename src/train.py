...
def prediction_step(sess, dataset, dataset_type, model, transition_params_trained,
  ...
  output_filepath = os.path.join(stats_graph_folder,
                                 '{1:03d}_{0}.txt'.format(dataset_type,
                                                          epoch_number))
  ...
  for i in range(len(dataset.token_indices[dataset_type])):
    ...
    unary_scores, predictions = sess.run([model.unary_scores, model.predictions],
                                         feed_dict)
    ...
    output_string = ''
    prediction_labels = [dataset.index_to_label[prediction] for prediction in
                         predictions]
    ...
    for prediction, token, gold_label in zip(prediction_labels,
      ...
      while True:
        ...
        if '-DOCSTART-' in split_line[0] or len(split_line) == 0 or len(
            split_line[0]) == 0:
          continue
        else:
