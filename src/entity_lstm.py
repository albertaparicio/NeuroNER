...
def bidirectional_LSTM(input, hidden_state_dimension, initializer,
  ...
  with tf.variable_scope("bidirectional_LSTM"):
    ...
    for direction in ["forward", "backward"]:
      with tf.variable_scope(direction):
        ...
        lstm_cell[direction] = tf.contrib.rnn.CoupledInputForgetGateLSTMCell(
            hidden_state_dimension, forget_bias=1.0, initializer=initializer,
            state_is_tuple=True)
        # initial state: http://stackoverflow.com/questions/38441589/tensorflow
        # -rnn-initial-state
    ...
    # sequence_length must be provided for tf.nn.bidirectional_dynamic_rnn due to
    # internal bug
    outputs, final_states = tf.nn.bidirectional_dynamic_rnn(
        lstm_cell["forward"], lstm_cell["backward"], input, dtype=tf.float32,
        sequence_length=sequence_length, initial_state_fw=initial_state["forward"],
    ...
    if output_sequence == True:
      ...
      # max pooling
      #             outputs_forward, outputs_backward = outputs
      #             output = tf.concat([outputs_forward, outputs_backward],
      # axis=2, name='output_sequence')
      #             output = tf.reduce_max(output, axis=1, name='output')
      # last pooling
      final_states_forward, final_states_backward = final_states
      output = tf.concat([final_states_forward[1], final_states_backward[1]], axis=1,
                         name='output')
...
class EntityLSTM(object):
  ...
  Then the character vector is concatenated with token embedding vector, which is
  input to another LSTM  followed by a CRF layer.
  """
  ...
  def __init__(self, dataset, parameters):
    ...
    if parameters['use_character_lstm']:
      ...
      with tf.variable_scope("character_embedding"):
        ...
        embedded_characters = tf.nn.embedding_lookup(
            self.character_embedding_weights, self.input_token_character_indices,
      ...
      with tf.variable_scope('character_lstm') as vs:
        character_lstm_output = bidirectional_LSTM(
            embedded_characters, parameters['character_lstm_hidden_state_dimension'],
            initializer, sequence_length=self.input_token_lengths,
    ...
    with tf.variable_scope("feedforward_after_lstm") as vs:
      W = tf.get_variable(
          ...
          initializer=initializer)
      b = tf.Variable(
          tf.constant(0.0, shape=[parameters['token_lstm_hidden_state_dimension']]),
          name="bias")
    ...
    if parameters['use_crf']:
      with tf.variable_scope("crf") as vs:
        ...
        unary_scores_with_start_and_end = tf.concat([self.unary_scores, tf.tile(
            tf.constant(small_score, shape=[1, 2]), [sequence_length, 1])], 1)
        start_unary_scores = [
          [small_score] * dataset.number_of_classes + [large_score, small_score]]
        end_unary_scores = [
          [small_score] * dataset.number_of_classes + [small_score, large_score]]
        self.unary_scores = tf.concat(
            [start_unary_scores, unary_scores_with_start_and_end, end_unary_scores],
            0)
        ...
        input_label_indices_flat_batch = tf.expand_dims(
            input_label_indices_flat_with_start_and_end, axis=0,
            name='input_label_indices_flat_batch')
        if self.verbose: print(
            'unary_scores_expanded: {0}'.format(unary_scores_expanded))
        if self.verbose: print('input_label_indices_flat_batch: {0}'.format(
    ...
    self.saver = tf.train.Saver(max_to_keep=parameters[
      'maximum_number_of_epochs'])  # defaults to saving all variables
  ...
  def load_pretrained_token_embeddings(self, sess, dataset, parameters,
    ...
    for token in dataset.token_to_index.keys():
      if token in token_to_vector.keys():
        ...
        'check_for_lowercase'] and token.lower() in token_to_vector.keys():
      ...
      elif parameters['check_for_lowercase'] and parameters[
        'check_for_digits_replaced_with_zeros'] and re.sub('\d', '0',
                                                           token.lower()) in \
          token_to_vector.keys():
        initial_weights[dataset.token_to_index[token]] = token_to_vector[
    ...
    print("number_of_token_original_case_found: {0}".format(
  ...
  def load_embeddings_from_pretrained_model(self, sess, dataset, pretraining_dataset,
    ...
    if embedding_type == 'token':
      ...
      initial_weights[dataset.PADDING_CHARACTER_INDEX] = \
        pretrained_embedding_weights[pretraining_dataset.PADDING_CHARACTER_INDEX]
  ...
  def restore_from_pretrained_model(self, parameters, dataset, sess,
    ...
    if pretraining_dataset.index_to_token == dataset.index_to_token and \
      ...
      self.saver.restore(sess,
                         pretrained_model_checkpoint_filepath)  # Works only when
      # the dimensions of tensor variables are matched.

    # If the token and character mappings are different between the pretrained
    # model and the current model
    else:

      # Resize the token and character embedding weights to match them with the
      # pretrained model (required in order to restore the pretrained model)
