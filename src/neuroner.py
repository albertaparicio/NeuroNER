...
class NeuroNER(object):
  ...
  def _load_parameters(self, parameters_filepath, arguments={}, verbose=True):
    ...
    """
    parameters = {
      'pretrained_model_folder'              : '../trained_models/quantico_model',
  ...
  @staticmethod
  def _get_valid_dataset_filepaths(parameters,
  ...
  def _check_parameter_compatiblity(parameters, dataset_filepaths):
    ...
    if parameters['train_model']:
      if 'train' not in dataset_filepaths or 'valid' not in dataset_filepaths:
        raise IOError(
            "If train_model is set to True, both train and valid set must "
            "exist "
      ...
      if 'train' in dataset_filepaths and 'valid' in dataset_filepaths:
        print(
            "WARNING: train and valid set exist in the specified dataset "
            "folder, but train_model is set to FALSE: {0}".format(
                parameters['dataset_text_folder']))
  ...
  def __init__(self,
    ...
    session_conf = tf.ConfigProto(
        ...
        # automatically choose an existing and supported device to run the
    ...
    with sess.as_default():
      ...
      if not parameters['use_pretrained_model']:
        ...
        self.transition_params_trained = np.random.rand(
            len(dataset.unique_labels) + 2, len(dataset.unique_labels) + 2)
      else:
    ...
    self.sess = sess
    self.stats_graph_folder = None  # To be defined in 'predict'
  ...
  def fit(self):
    ...
    results = {
      ...
      'model_options'    : copy.copy(parameters)
      }
    ...
    embedding_writer = tf.summary.FileWriter(
        model_folder)  # embedding_writer has to write in model_folder,
    # otherwise TensorBoard won't be able to view embeddings

    embeddings_projector_config = projector.ProjectorConfig()
    tensorboard_token_embeddings = embeddings_projector_config.embeddings.add()
    tensorboard_token_embeddings.tensor_name = \
      model.token_embedding_weights.name
    ...
    tensorboard_character_embeddings = \
    ...
    bad_counter = 0  # number of epochs with no improvement on the validation
    #  test in terms of F1-score
    ...
    try:
      while True:
        ...
        print('Training completed in {0:.2f} seconds'.format(
            epoch_elapsed_training_time), flush=True)
        ...
        # Save model
        model.saver.save(sess, os.path.join(model_folder,
                                            'model_{0:05d}.ckpt'.format(
                                                epoch_number)))
        ...
        if valid_f1_score > previous_best_valid_f1_score:
          ...
          bad_counter += 1
        print(
            "The last {0} epochs have not shown improvements on the validation "
            "set.".format(
                bad_counter))
  ...
  def predict(self, text):
    ...
    if self.prediction_count == 1:
      self.parameters['dataset_text_folder'] = os.path.join('..', 'data',
                                                            'temp')
    ...
    dataset_brat_deploy_filepath = os.path.join(dataset_brat_deploy_folder,
                                                'temp_{0}.txt'.format(str(
                                                    self.prediction_count).zfill(
                                                    5)))  #
    ...
    output_filepaths = {}
    prediction_output = train.prediction_step(self.sess, self.dataset,
                                              dataset_type, self.model,
