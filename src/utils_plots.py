...
def get_cmap():
  """
  http://stackoverflow.com/questions/37517587/how-can-i-change-the-intensity-of-a
...
def heatmap(AUC, title, xlabel, ylabel, xticklabels, yticklabels, figure_width=40,
            figure_height=20, correct_orientation=False, cmap='RdBu', fmt="%.2f",
            graph_filepath='', normalize=False, remove_diagonal=False):
  """
  Inspired by:
...
def plot_classification_report(classification_report, title='Classification report ',
                               cmap='RdBu', from_conll_json=False):
  """
  ...
  if from_conll_json:
    for label in sorted(classification_report.keys()):
      ...
      plotMat.append([float(classification_report[label][x]) for x in
                      ["precision", "recall", "f1"]])
  else:
...
def plot_hist(sequence, xlabel, ylabel, title, graph_path):
  ...
  xmax = max(sequence)
  step = 1
  y, x = np.histogram(sequence,
                      bins=np.linspace(xmin, xmax, (xmax - xmin + 1) / step))
...
def plot_barh(x, y, xlabel, ylabel, title, graph_path):
  ...
  # http://stackoverflow.com/questions/30228069/how-to-display-the-value-of-the
  # -bar-on-each-bar-with-pyplot-barh/30229062#30229062
  ...
  plt.savefig(graph_path, dpi=300, format='png',
              bbox_inches='tight')  # use format='svg' or 'pdf' for vectorial
  # pictures
  plt.clf()
  plt.close()
