...
class Dataset(object):
    ...
    for dataset_type in dataset_types:
      ...
      for token_sequence in tokens[dataset_type]:
        token_indices[dataset_type].append(
            [token_to_index.get(token, self.UNK_TOKEN_INDEX) for token in
             token_sequence])
  ...
  def update_dataset(self, dataset_filepaths, dataset_types):
    ...
    Overwrites the data of type specified in dataset_types using the existing
    token_to_index, character_to_index, and label_to_index mappings.
    '''
    for dataset_type in dataset_types:
  ...
  def load_dataset(self, dataset_filepaths, parameters, token_to_vector=None):
    ...
    # Load pretraining dataset to ensure that index to label is compatible to the
    # pretrained model,
    #   and that token embeddings that are learned in the pretrained model are
    # loaded properly.
    ...
    for dataset_type in ['train', 'valid', 'test', 'deploy']:
      labels[dataset_type], tokens[dataset_type], token_count[dataset_type], \
      label_count[dataset_type], character_count[dataset_type] \
    ...
    for token in list(token_count['train'].keys()) + list(
        token_count['valid'].keys()) + list(token_count['test'].keys()) + list(
        token_count['deploy'].keys()):
      token_count['all'][token] = token_count['train'][token] + token_count['valid'][
        token] + token_count['test'][token] + token_count['deploy'][token]
    ...
    for character in list(character_count['train'].keys()) + list(
      ...
      character_count['all'][character] = character_count['train'][character] + \
    ...
    for character in list(label_count['train'].keys()) + list(
        label_count['valid'].keys()) + list(label_count['test'].keys()) + list(
        label_count['deploy'].keys()):
      label_count['all'][character] = label_count['train'][character] + \
                                      label_count['valid'][character] + \
                                      label_count['test'][character] + \
                                      label_count['deploy'][character]

    token_count['all'] = utils.order_dictionary(token_count['all'], 'value_key',
                                                reverse=True)
    label_count['all'] = utils.order_dictionary(label_count['all'], 'key',
                                                reverse=False)
    ...
    for token, count in token_count['all'].items():
      ...
      if parameters['remap_unknown_tokens_to_unk'] == 1 and \
          ...
          not utils_nlp.is_token_in_pretrained_embeddings(token, token_to_vector,
                                                          parameters) and \
    ...
    if self.verbose: print('token_count[\'train\'][0:10]: {0}'.format(
    ...
    if parameters['remap_unknown_tokens_to_unk'] == 1: index_to_token[
      self.UNK_TOKEN_INDEX] = self.UNK
    ...
    character_to_index = utils.order_dictionary(character_to_index, 'value',
                                                reverse=False)
