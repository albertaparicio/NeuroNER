"""
This script prepares a pretrained model to be shared without exposing the data
used for training.
"""
import glob
import os
...
def trim_dataset_pickle(input_dataset_filepath, output_dataset_filepath=None,
                        delete_token_mappings=False):
  """
  Remove the dataset and labels from dataset.pickle.
  If delete_token_mappings = True, then also remove token_to_index and
  index_to_token except for UNK.
  """
  ...
  keys_to_remove = ['character_indices', 'character_indices_padded', 'characters',
                    'label_indices', 'label_vector_indices', 'labels',
                    'token_indices', 'token_lengths', 'tokens',
                    'infrequent_token_indices', 'tokens_mapped_to_unk']
  ...
  if delete_token_mappings:
    dataset.__dict__['token_to_index'] = {
      dataset.__dict__['UNK']: dataset.__dict__['UNK_TOKEN_INDEX']}
    dataset.__dict__['index_to_token'] = {
      dataset.__dict__['UNK_TOKEN_INDEX']: dataset.__dict__['UNK']}
...
def trim_model_checkpoint(parameters_filepath, dataset_filepath,
  ...
  with tf.Session() as sess:
    ...
    model_saver.restore(sess,
                        input_checkpoint_filepath)  # Works only when the
    # dimensions of tensor variables are matched.
    ...
    initial_weights[dataset.UNK_TOKEN_INDEX] = token_embedding_weights[
      dataset.UNK_TOKEN_INDEX]
    sess.run(tf.assign(model.token_embedding_weights, initial_weights,
...
def check_contents_of_dataset_and_model_checkpoint(model_folder):
  ...
  with tf.Session() as sess:
    print_tensors_in_checkpoint_file(
        checkpoint_filepath,
        tensor_name='token_embedding/token_embedding_weights',
        all_tensors=True,
        all_tensor_names=None)
    print_tensors_in_checkpoint_file(
        checkpoint_filepath,
        tensor_name='token_embedding/token_embedding_weights',
        all_tensors=False, all_tensor_names=None)
